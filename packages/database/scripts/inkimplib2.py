...
missing_legacyIds_in_ddb1 = []
explicit_uuids_in_import = []
new_uuids_in_import = []

def get_uuid_id2(table_row, legacy_to_uuid):
  if "ddb1_uuid" in table_row and table_row["ddb1_uuid"]!="" and is_valid_uuid(table_row["ddb1_uuid"]):
    explicit_uuids_in_import.append((table_row["legacyId"],table_row["ddb1_uuid"]))
    return table_row["ddb1_uuid"]
  elif "legacyId" in table_row:
    if table_row["legacyId"] in legacy_to_uuid.keys():
      return legacy_to_uuid[table_row["legacyId"]]
    else:
      missing_legacyIds_in_ddb1.append(table_row["legacyId"])
      logger.warning("legacy id %s not found in ddb1 data", table_row["legacyId"])
      return str(uuid.uuid4())
  else:
    new_uuid =  str(uuid.uuid4())
    new_uuids_in_import.append((table_row["legacyId"], new_uuid))
    return new_uuid
...
class InkVisitorJSONObjectFactory:
  ...
  json_class_defaults = {
    'IAction': {
      ...
      'data': {'entities': {'a1': [], 'a2': [], 's': []}, 'valencies': {'a1': '', 'a2': '', 's': ''}, "pos":"verb"}, 'props': [],
      'notes': [], 'status': '1', 'references': []
    },
    'IConcept': {
      'class': 'C', 'id': '', 'legacyId': '', 'label': '', 'language': '', 'detail': '', 'data': {'pos':''}, 'props': [],
      'notes': [], 'status': '1', 'references': []
    },
    'IValue': {
...
class EntityMapper:
  ...
  'indefinite' : "2",   'hypothetical' : "3",  'generic' : "4"}}
    ...
    IOF = InkVisitorJSONObjectFactory(schema_path, "Import batch [ReImport 06/2024] " + str(datetime.now()))
    ...
    def make_ventity(self, value_string, origin=""):
        ...
        # register ventity
        self.dh.tables['values'] = self.dh.tables['values']._append({'id':ventity['id'] ,'value':ventity['label'],"origin":origin},ignore_index=True )
    ...
    def make_rentity(self, label, url = "", origin="", legacyId = ""):
        ...
        # register rentity
        self.dh.tables['resources'] = self.dh.tables['resources']._append({'id':rentity['id'] ,'label':rentity['label'],"dissinet_repository_url":url, "origin":origin}, ignore_index=True )
    ...
    def make_eentity(self, label, legacyId, url = "", origin=""):
        ...
        # register event
        self.dh.tables['events'] = self.dh.tables['events']._append({'id':eentity['id'] ,'value':eentity['label'],"origin":origin, "legacyId":legacyId}, ignore_index=True )
    ...
    def make_lentity(self, label, legacyId="", url = "", origin=""):
        ...
        # register lentity
        self.dh.tables['locations'] = self.dh.tables['locations']._append({'id':lentity['id'] ,'value':lentity['label'],"origin":origin,"legacyId":legacyId}, ignore_index=True )
    ...
    def make_tentity(self, label, legacyId="", url = "", origin=""):
        ...
        # register tentity
        self.dh.tables['territories'] = self.dh.tables['territories']._append({'id':tentity['id'] ,'value':tentity['label'],"origin":origin,"legacyId":legacyId}, ignore_index=True)
        if legacyId != "":
    ...
    def make_relation_identity_record(self, id1, id2, certainty="", origin="", order=1):
        ...
        # register relation
        self.dh.tables['relations'] = self.dh.tables['relations']._append({'id':rir['id'] ,'type':rir['type'], "certainty":rir['certainty'],"entityIds":rir['entityIds'], "origin":origin}, ignore_index=True )
        rir = eval(str(rir))
    ...
    def make_relation_classification_record(self, id1, id2, origin="", order=1):
        ...
        # register relation
        self.dh.tables['relations'] = self.dh.tables['relations']._append({'id':rcr['id'] ,'type':rcr['type'], "entityIds":rcr['entityIds'], "origin":origin}, ignore_index=True )
        rcr = eval(str(rcr))
    ...
    def make_relation_record(self, relation_type, ids_list, origin="", order=1):
        ...
        # register lentity
        self.dh.tables['relations'] = self.dh.tables['relations']._append({'id':rr['id'] ,'type':rr['type'], "entityIds":rr['entityIds'], "origin":origin}, ignore_index=True )
        rr = eval(str(rr))
    ...
    def update_label_language(self, field_name="", input_value="", origin = ""):
        if input_value in type(self).enum_mapper['language']:
            ...
            self.logger.warning(f"Unable to set language in {origin}.")
            #self.entity['language'] = input_value # will raise error
            self.entity['language'] = '' # will raise error
...
class TextParser(Parser):
    ...
    def special_ddb1_uuid(self, operation, value, entity_mapper, field_name="special_ddb1_uuid"):
      pass
...
class EventParser(Parser):
    ...
    def special_subterritory_id(self, operation, value, entity_mapper, field_name = "subterritory_id"):
        ...
        trt.make_alive(legacyId= datarow["subterritory_id"], label=datarow['subterritory_label'], label_language=datarow['document_label_language'], register_id_where="texts")
